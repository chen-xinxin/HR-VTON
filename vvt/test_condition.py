...
def get_opt():
    ...
    parser.add_argument("--dataroot", default="./data/192_256")
    ...
    parser.add_argument('--D_checkpoint', type=str, default='./checkpoints/train/D_final.pth', help='D checkpoint')
...
def test(opt, test_loader, board, tocg, D=None):
    ...
    grid_dir = os.path.join('./output', opt.datamode, opt.datasetting, 'test_condition')
    ...
    if D is not None:
        ...
        for name, score in D_score:
            f = open(os.path.join('./output', opt.datamode, opt.datasetting, 'test_condition', 'rejection_prob.txt'), 'a')
