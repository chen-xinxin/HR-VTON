...
class CPDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        for key in ['paired']:
            ...
            c[key] = Image.open(osp.join(self.data_path, 'cloth-and-mask', c_name[key])).convert('RGB')
            input_tensor = TF.to_tensor(c[key])
            output_tensor = F.interpolate(input_tensor.unsqueeze(0), scale_factor=2.0, mode='nearest')
            c[key] = TF.to_pil_image(output_tensor.squeeze(0))
            c[key] = transforms.Resize(self.fine_width, interpolation=2)(c[key])
            # cm[key] = Image.open(osp.join(self.data_path, 'cloth-mask', c_name[key].replace('jpg','png')))
            cm[key] = Image.open(osp.join(self.data_path, 'cloth-and-mask', c_name[key].replace('cloth_front','cloth_front_mask')))
...
#     parser.add_argument("--dataroot", default="./data/dresscode")
