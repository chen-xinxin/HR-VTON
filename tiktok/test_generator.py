...
def get_opt():
    ...
    parser.add_argument("--dataroot", default="./data/tiktok")
    parser.add_argument("--datamode", default="test")
    parser.add_argument("--data_list", default="test_unpairs.txt")
    parser.add_argument("--output_dir", type=str, default="./tiktok_unpaired_output")
        ...
        for inputs in test_loader.data_loader:
            ...
            for i in range(shape[0]):
                grid = make_image_grid([(clothes[i].cpu() / 2 + 0.5), (pre_clothes_mask[i].cpu()).expand(3, -1, -1), visualize_segmap(parse_agnostic.cpu(), batch=i), ((densepose.cpu()[i]+1)/2),
                                        ...
                                        nrow=4)
                unpaired_name = inputs['im_name'][i]
                print(unpaired_name)
