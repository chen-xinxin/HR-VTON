...
class CPDatasetTest(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        for key in self.c_names:
            c_name[key] = self.c_names[key][index]
            c[key] = Image.open(osp.join(self.data_path, 'cloth', c_name[key].replace('0.jpg','1.jpg'))).convert('RGB')
            c[key] = transforms.Resize(self.fine_width, interpolation=2)(c[key])
            cm[key] = Image.open(osp.join(self.data_path, 'cloth-mask', c_name[key].replace('0.jpg','1.jpg').replace('jpg','png')))
            cm[key] = transforms.Resize(self.fine_width, interpolation=0)(cm[key])
